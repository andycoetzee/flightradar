...
public:
  ...
  // Expose the underlying esp_lcd panel handle so the sketch can call
  // esp_lcd_rgb_panel_restart() to recover from a DMA de-sync (permanent image
  // shift) on PSRAM bandwidth starvation. (Local addition - re-apply if this
  // library is updated.)
